...
const SlotMachine = forwardRef<SlotMachineRef>((props, ref) => {
  ...
  const getSymbolHeight = (): number => {
    ...
    if (vw < 768) {
      return 80; // Phone: 80px reel = 80px symbol height (better mobile fit)
    } else if (vw < 1024) {
  ...
  // Update the spin function with responsive calculations
  const spin = async () => {
    setIsSpinning(true);
    ...
    reelsRef.current.forEach((reelRef, index) => {
      ...
      let position = 0;
      const speed = 30 + index * 6; // Doubled the speed values (was 15 + index * 3)
    ...
    for (let i = 0; i < reelsRef.current.length; i++) {
      await sleep(400); // Delay between reel stops
